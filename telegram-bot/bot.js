...
if (!TELEGRAM_BOT_TOKEN) {
  logger.error(
    "TELEGRAM_BOT_TOKEN is not set. Please configure your .env file.",
  );
  process.exit(1);
}
...
bot.on("photo", async (msg) => {
  ...
  try {
    ...
    if (missingFields.length > 0) {
      logger.warn(
        `[photo] Missing fields in analysis: ${missingFields.join(", ")}`,
      );
    ...
    if (analysis.confidence != null) {
      ...
      if (!isNaN(raw)) {
        confidencePercent =
          raw <= 1 ? `${(raw * 100).toFixed(1)}%` : `${raw.toFixed(1)}%`;
      }
    }
...
if (BOT_MODE === "webhook") {
  ...
  // ── Polling mode: standard long-poll + minimal HTTP server for health checks ─
  // Even in polling mode, we need HTTP server for Docker healthcheck
  const app = express();
  app.use(express.json());

  // Health check endpoint (required for Docker healthcheck)
  app.get("/health", (_req, res) => {
    res.json({ status: "ok", service: "skinx-telegram-bot", mode: "polling" });
  });

  const port = parseInt(BOT_PORT, 10);
  app.listen(port, () => {
    logger.info(`SkinX Telegram Bot health server started on port ${port}`);
  });
